...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {
		size(1024, 768, OPENGL);
		  ...
		  square.setFill(color(128,128,128));
		  square.setStroke(false);

		if (offline) {
		    map = new UnfoldingMap(this, 0, 0, 2000,2000 , new MBTilesMapProvider(mbTilesString));
	    ...
	    if (earthquakes.size() > 0) {
	    	for ( PointFeature earthquake : earthquakes)
	    	{
		    	...
		    	if(mag > 4.0f && mag <= 4.9f)
		    	{ 
		    		spm.setColor(color(210, 227, 2));
		    		spm.setRadius(mag*1.5f);
		    	}
		    	if(mag > 4.9f)
		    	{ 
